...
const isFinished = (date) => {
  const [monthStr, days] = date.split(' ');
  ...
  const raceEnd = new Date(2026, monthMap[monthStr], parseInt(endDay));
  return new Date() > raceEnd;
};

const GrandPrix = ({ round, country, title, date, trackImg, flag }) => {

  const finished = isFinished(date);
